...
const Hero: React.FC = () => {
  ...
  const phrase = "Extractores industriales Costa Rica";
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10 w-full">
        <div className="max-w-4xl">
          <Reveal width="100%" delay={200}>
            <h1 className="text-4xl md:text-6xl font-black text-white leading-[0.9] uppercase mb-10 min-h-[3.6em] md:min-h-[1.8em] flex items-center">
              <span className="inline-flex flex-wrap">
                {text.split(' ').map((word, i, arr) => (
                  <span key={i} className={i >= 2 ? 'text-[#E62E2E]' : 'text-white'}>
                    {word}{i !== arr.length - 1 ? '\u00A0' : ''}
                  </span>
                ))}
              </span>
